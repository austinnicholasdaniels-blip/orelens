...
TOTAL_PAT = re.compile(
    r"(?:aggregate|total(?: consideration| fee| cost)? of|budget of|consideration of)\s*(?:up to\s*)?"
...
def parse_promotion(text: str) -> dict | None:
    ...
    firm = None
    fm = FIRM_PAT.search(text) or FIRM_PAT2.search(text)
    if fm:
